...
help_single_analysis <- function(filtFs, sName, index, config) {
        ...
        if (!is.null(object@dadaBandSize) & !is.null(object@dadaBandSize)) {
                ...
                dadaFs <- dada2::dada(derepFs, err=errFs, BAND_SIZE=object@dadaBandSize, OMEGA_A=object@dadaOmegaA)
        }
        ...
        saveRDS(seqtab.nochim, file=seq.print)

        # Classify sequences
        message("\n==== Assigning Taxa ====")
        taxa <- dada2::assignTaxonomy(seqtab.nochim, taxDatabase, minBoot = 80, multithread=TRUE)
        f.print <- paste0(index, "_", sName, "_OTU_Table.csv")

        # Save OTU table
        write.table(taxa, file = paste0(f.print))
...
single_analysis <- function(fp, sample.names, file, getQuality = FALSE, REGEX = "*.fastq(.gz)?$") {
        ...
        if (getQuality) {
                ...
                # Finish analysis
                errs <- lapply(filtFs, help_single_analysis, sName=sample.names, index=1:length(sample.names), config=file)
        }


help_paired_analysis <- function(filtFs, filtRs, sName, index, config) {
        if (is.null(filtFs)) {
        ...
        seqtab.nochim <- dada2::removeBimeraDenovo(seqtab, method="consensus", multithread=FALSE, verbose=TRUE)
                if (options$createChimeraDetectionTable) {
                        ...
                        saveSeqs(seqtab.chim, sName, index, options$outDir, add.table = TRUE, options$projectPrefix)
                }

        # Save Sequence Table without chimeras
        seq.print <- saveSeqs(seqtab.nochim, sName, index, options$outDir, add.table = FALSE, options$projectPrefix)
        message("Created: ", seq.print)
        ...
        track <- cbind(sName, forward.derep, reverse.derep, forward.dada, reverse.dada, merged)
...
paired_analysis <- function(fp, sample.names, file, getQuality = FALSE, REGEX_1 = "*_1.fastq(.gz)?$", REGEX_2 = "*_2.fastq(.gz)?$") {
        ...
        # TODO: Test all inputs in config file
        infoList <- lapply(1:length(Fs), function(input, samples, label, config) {
                return(help_paired_analysis(filt.forward[[input]], filt.reverse[[input]], sName=samples[input], index=input, config=config))
                },  samples=sample.names, label=1:length(sample.names), config=file)
		...
		if (file.exists("filtered_objects.RData")) {
                ...
                # Combine all information
                track <- cbind(sample_ls, saveFilt, derepFs_ls, derepRs_ls, dadaFs_ls, dadaRs_ls, merged_ls)
