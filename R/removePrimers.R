...
#' @param object An S4 object of type fastq2otu (i.e. fastq2otu_single or fastq2otu_paired).
...
removePrimers <- function(object) {
	...
	samples <- object@listOfSamples
	check <- ifelse(file.exists(samples), TRUE, FALSE)
	ifelse(check, check, stop(paste0(samples, " could not be found. Please provide a valid file path to your samples."))
